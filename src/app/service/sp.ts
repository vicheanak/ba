...
export class SpService {
    ...
    getAll() {
        return new Promise(resolve => {
            ...
            this.http.get(this.host + '/sps', options)
                .subscribe(data => {
                    resolve(data.json());
                });
        });
    ...
    get(spId){
        return new Promise(resolve => {
            ...
            let options = new RequestOptions({ headers: headers });
            this.http.get(this.host + '/sps/' + spId, options)
                .subscribe(data => {
                    resolve(data.json());
                });
        });
    ...
    insert(user){
        return new Promise(resolve => {
            ...
            this.http.post(this.host + '/sps', body, options)
                .subscribe(data => {
    ...
    update(user){
        return new Promise(resolve => {
            ...
            let options = new RequestOptions({ headers: headers });
            let body = JSON.stringify(user);
            this.http.put(this.host + '/sps/' + user.sp.id, body, options)
                .subscribe(data => {
                    this.cbData = data.json();
                    if (this.cbData.success == false){
                        this.router.navigate(['/login']);
    ...
    delete(product){
        return new Promise(resolve => {
            ...
            this.http.delete(this.host + '/sps/' + product.id, options)
                .subscribe(data => {
                    this.cbData = data.json();
                    if (this.cbData.success == false){
                        this.router.navigate(['/login']);
